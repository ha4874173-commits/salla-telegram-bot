...
# --- 2. لوحة الأزرار (نفس القديمة + زر التأكيد) ---
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'menu_spx':
        ...
        await query.edit_message_text("باقات اشتراك SPX:", reply_markup=InlineKeyboardMarkup(keyboard))
        ...
        await query.message.reply_text("✅ أرسل الآن صورة الفاتورة أو رقم الطلب ليتم مراجعته من قبل الإدارة.")

    elif data == 'back_main':
        await query.edit_message_text("القائمة الرئيسية:", reply_markup=main_menu_keyboard())

# --- 4. معالج طلبات العملاء (إرسال لفيصل) ---
async def handle_verification_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # الإرسال لفيصل (ADMIN_ID)
    try:
        if update.message.photo:
            await context.bot.send_photo(chat_id=ADMIN_ID, photo=update.message.photo[-1].file_id, 
                                       caption=f"📩 طلب جديد من: {user.first_name}\nID: `{user.id}`", 
                                       reply_markup=InlineKeyboardMarkup(keyboard))
        else:
            await context.bot.send_message(chat_id=ADMIN_ID, 
                                         text=f"📩 طلب جديد (نصي) من: {user.first_name}\nالرسالة: {update.message.text}\nID: `{user.id}`", 
                                         reply_markup=InlineKeyboardMarkup(keyboard))
        
        await update.message.reply_text("✅ تم إرسال طلبك للمراجعة، انتظر الرد.")
    except Exception as e:
        print(f"Admin Not Found: {e}")

# --- 5. معالج قرار الأدمن (القبول/الرفض) ---
async def admin_decision_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "approve":
        await context.bot.send_message(chat_id=user_id, text=f"🎉 تهانينا! تم تفعيل اشتراكك.\nرابط القناة الخاصة:\n{URLS['private_channel']}")
        await context.bot.send_message(chat_id=DATA_CHANNEL_ID, text=f"✅ تفعيل يدوي: تم قبول {user_id}")
        res = "✅ تم القبول"
    else:
